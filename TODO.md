...
> 最后更新: 2025-12-28 (Phase 8 历史记录功能 - 100% 完成 ✅)
...
> **详细实施计划**: 见 [PHASE9_PLAN.md](./PHASE9_PLAN.md)
> **技术要求**: 见 [CLAUDE.md - Phase 9](./CLAUDE.md#phase-9-代码重构与架构优化)

### 重构数据概览

**基于 2025-12-28 代码分析结果**:
- **核心 UI 代码**: 2,995 行
- **重复代码量**: ~700-750 行（23-25%）
- **高度重复**: 254 行（95% 相似度，可直接提取）
- **中度重复**: 400-450 行（60-70% 相似度，需参数化）
...
- [ ] 提取 **ModeNavigationBar** 组件（2 处复用，-30 行）
- [ ] 提取 **ConfirmDialog** 组件（7 处复用，-180 行）
- [ ] 测试并提交
...
- [ ] 提取 **EditRecordDialog** 通用组件（2 处复用，-120 行）
- [ ] 支持 TimeRecord 和 TimeRecordEntity 两种类型
- [ ] 测试并提交
...
- [ ] 创建 **RecordCardMode** 枚举（4 种显示模式）
- [ ] 提取 **UnifiedRecordCard** 组件（3 处复用，-150 行）
- [ ] 测试并提交
...
- [ ] 重构 MainScreen.kt（使用 ModeNavigationBar）
- [ ] 重构 EventScreen.kt（使用 ConfirmDialog + UnifiedRecordCard，-51 行）
- [ ] 重构 StopwatchScreen.kt（使用所有组件，移动 ControlButton，-148 行）
- [ ] 重构 HistoryScreen.kt（使用所有组件，-242 行）
- [ ] 测试并提交
...
- [ ] 全面功能测试（事件、秒表、历史、设置）
- [ ] 代码质量检查（Lint）
- [ ] 更新 CLAUDE.md（项目结构、已完成功能）
- [ ] 更新 TODO.md（标记 Phase 9 完成）
- [ ] 最终提交并打标签 `v0.9-phase9`
...
ui/components/
├── navigation/
│   └── ModeNavigationBar.kt          (40 行，事件/秒表导航栏)
├── record/
│   ├── RecordCardMode.kt             (10 行，卡片模式枚举)
│   └── UnifiedRecordCard.kt          (90 行，统一记录卡片)
├── button/
...
| UI 一致性 | 70% | 95% | **+25%** |
| 代码复用率 | 0% | 40%+ | **+40%** |
| Screen 总行数 | 2995 行 | 2527 行 | **-468 行 (-16%)** |
