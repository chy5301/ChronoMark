 ...
 *
 * **功能**：
 * - 显示只读信息：序号、标记时刻（秒表模式额外显示累计时间）
 * - 编辑备注文字（最多 8 行，最少显示 3 行）
 * - 保存/取消/删除操作
 *
 * **使用场景**：
 * - StopwatchScreen: 编辑工作区记录（秒表模式）
 * - EventScreen: 编辑工作区事件（事件模式）
 * - HistoryScreen: 编辑历史记录（根据模式区分）
 *
