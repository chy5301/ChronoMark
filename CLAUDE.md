...
14. 创建 EventViewModel 管理事件模式状态
15. 实现事件模式时间显示区（仅墙上时钟）
16. 实现事件模式记录卡片（标记时刻 + 时间差）
17. 实现事件模式控制按钮（记录 + 重置）
18. 两种模式数据分开存储

#### Phase 5: 数据持久化
19. 集成 DataStore 实现状态保存
20. 实现应用重启后状态恢复
21. 处理边缘情况（应用被杀死等）

#### Phase 6: 导出功能
22. 实现 ExportHelper 工具类
23. 实现 CSV 导出
24. 实现 JSON 导出
25. 实现 TXT 导出
26. 处理 Android 存储权限（Scoped Storage）
